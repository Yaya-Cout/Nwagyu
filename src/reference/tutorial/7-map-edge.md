...
Technically, storing -1 in an `uint16_t` is undefined behavior, which means the
...
Undefined behavior was made to ease C compiler writing, but is now one of the
...
If you are interested in details about undefined behavior, this blog post is
...
The game currently prevents you from killing yourself by doing a U-turn. The
...
fixing bugs will allow us to focus on other things.
